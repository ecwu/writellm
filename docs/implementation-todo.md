...
Checkpoint 26.9 has resumed with tag-only four-platform CI; `.32` passed macOS x64 and Linux
credential preflight, and its hosted Windows timing plus macOS selection failures are corrected for
candidate `.33`.
Recorded: 2026-08-29
...
- [~] Checkpoint 26.9: run the restored tag-only Windows x64, macOS arm64, macOS x64, and Linux x64
  validation/package matrix and record its hosted evidence. Candidate `.32` passed static,
  credential-preflight, and macOS x64; corrected candidate `.33` adds a hosted Windows Vitest budget
  and stable keyboard focus/selection transitions. Release promotion remains disabled.
