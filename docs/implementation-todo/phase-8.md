...
- [x] Carry and validate the active `projectSessionId` on every Index utility request and response, rejecting delayed messages after close or project switch; verify logical source/chunk fingerprints on open, not only SQLite structural integrity. Correction (2026-07-16 implementation audit): both behaviors are implemented, but there is no direct unit test for `IndexClient`'s stale-session/unmatched-response termination paths — coverage is indirect via search IPC, job supervisor, and E2E tests. The direct protocol test is backfilled under Checkpoint 19.7.
...
- [x] Resolve citation assets by the citation's `parseRevisionId`, not the knowledge item's independently changing active parse revision. Correction (2026-07-16 implementation audit): the by-citation-revision resolution is implemented; the dedicated parse/index activation-race test this item also promised was not found and is backfilled under Checkpoint 19.7.
...
- [x] Add packaged embedding/rerank hybrid-search coverage, including provider failure fallback, stale sessions, filtered retrieval, and text/image citation provenance. Correction (2026-07-16 implementation audit): delivered evidence is the Electron-run `smoke:packaged-hybrid` active-generation FTS+vector query plus unpackaged E2E for rerank-unconfigured fallback and text/image citations; packaged provider-failure-fallback and stale-session scenarios are not covered, and the smoke builds its index schema directly rather than driving the app's `IndexDatabase`. The missing packaged scenarios are backfilled under Checkpoint 19.7.
