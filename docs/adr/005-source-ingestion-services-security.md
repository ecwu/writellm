...
- Status: Accepted v1.2 — original decision, local original-PDF preview amendment, and fixed SiliconFlow China endpoint accepted by maintainer 2026-07-13
- Date: 2026-07-13
- Owners: WriteLLM v2 maintainers
- Scope: `006-source-library-processing` MinerU parsing, SiliconFlow embedding, protected credentials, transport/data egress, and current-version-fenced local original-PDF preview
...
4. SiliconFlow transport is a narrow main-only China-region adapter fixed to `https://api.siliconflow.cn/v1/embeddings`, bearer authentication, `model: "BAAI/bge-m3"` and `encoding_format: "float"`. Credential validation is fixed to `https://api.siliconflow.cn/v1/models`. The product does not expose an endpoint or region selector and never sends the credential or eligible block text to `api.siliconflow.com`. It receives only bounded eligible block text. Main requires exact request/response correspondence, finite 1024-dimensional vectors and the current index profile before persistence.
...
10. Local renderer preview of a canonical imported PDF stays inside the 006 source-content boundary. Main serves only the current active project/session/source/version through the exact reserved `writellm-source://<sourceId>/__original__/<sourceVersionId>.pdf` route after fixed-path, regular-file, signature, size, and stored-hash verification.
...
- Expose absolute paths, generic file IPC, whole-file preload bytes, `file://`, `<embed>/<iframe>`, or Electron PDF plugins for preview: rejected because each grants broader renderer capability or weaker fencing than the accepted fixed protocol plus PDF.js display layer.
...
- [x] Maintainer accepts the exact active-session/current-version-fenced original-PDF route, bounded range streaming, safe error/CSP headers, and no path/preload-byte exposure.
- [x] Maintainer accepts locally bundled PDF.js display rendering with no plugin, remote-resource, document-script, attachment, form, print/export, or expanded scheme privilege.
