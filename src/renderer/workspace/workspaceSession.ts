...
export type StatusState = 'in-progress' | 'complete' | 'error' | 'needs-action' | 'unknown' | 'owner-unavailable';
...
export function createWorkspaceSession(project: ProjectSnapshot): WorkspaceSession {
  return { project, activePanelId: null, panelMode: null, panelFocusReturnKey: null, latestStatusSequenceBySource: new Map(), statusBySource: new Map() };
}

function validStatus(summary: OwnerStatusSummary): boolean {
  if (!summary.sourceId.trim() || !summary.message.trim() || !Number.isSafeInteger(summary.sequence) || summary.sequence < 0) return false;
...
export function workspaceSessionReducer(session: WorkspaceSession, event: WorkspaceSessionEvent): WorkspaceSession {
  if (event.type === 'panel.preview') return { ...session, activePanelId: event.id, panelMode: 'preview', panelFocusReturnKey: event.focusReturnKey };
  if (event.type === 'panel.pin') {
    if (session.activePanelId === event.id && session.panelMode === 'pinned') return { ...session, activePanelId: null, panelMode: null, panelFocusReturnKey: null };
    return { ...session, activePanelId: event.id, panelMode: 'pinned', panelFocusReturnKey: event.focusReturnKey };
  }
...
const priority: Record<StatusState, number> = { error: 6, 'needs-action': 5, 'owner-unavailable': 4, unknown: 3, 'in-progress': 2, complete: 1 };
export function selectPrimaryStatus(statuses: Iterable<OwnerStatusSummary>): OwnerStatusSummary | null {
  return [...statuses].sort((a, b) => priority[b.state] - priority[a.state] || b.sequence - a.sequence || a.sourceId.localeCompare(b.sourceId))[0] ?? null;
}
