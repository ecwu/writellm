...
export const migration0038: DatabaseMigration = {
  version: 38,
  name: '0038-native-block-math-diagram-schema-v5',
  checksum: 'sha256:ca8148ba0d147c3c7ce8e414b3d04d8cf65f01ba70676747bf3ae50f4b3186d1',
  up(database) {
    ...
    try {
      ...
      if (DEPENDENT_TABLES.some((name) => !tableDefinitions.has(name))) {
        throw new Error(
          'Native block Math and Diagram migration found an unexpected project schema'
        )
      }
      ...
      for (const table of RENAME_ORDER) {
        database.exec(`ALTER TABLE "${table}" RENAME TO "${table}_v37"`)
      }
      database.exec('ALTER TABLE section_revisions RENAME TO section_revisions_v37')
      ...
      database.exec(`
        CREATE TABLE section_revisions (
          ...
          content_schema_version INTEGER NOT NULL CHECK (content_schema_version IN (1, 2, 3, 4, 5)),
      ...
      database.exec('INSERT INTO section_revisions SELECT * FROM section_revisions_v37')
      database.exec('INSERT INTO sections SELECT * FROM sections_v37')
      for (const table of RECREATE_ORDER.filter((name) => name !== 'sections')) {
        database.exec(tableDefinitions.get(table) as string)
        database.exec(`INSERT INTO "${table}" SELECT * FROM "${table}_v37"`)
      }

      for (const table of RENAME_ORDER) database.exec(`DROP TABLE "${table}_v37"`)
      database.exec('DROP TABLE section_revisions_v37')
      ...
      const current = database
        .prepare(
          `SELECT revision.section_id, revision.section_revision_id,
                  ...
                  revision.content_json, revision.content_schema_version,
      ...
      for (const revision of current) {
        if (revision.content_schema_version !== 4 || revision.count_algorithm_version !== 2) {
          throw new Error('Native block Math and Diagram migration found a stale active revision')
        }
        const prepared = prepareSectionContent(
          projectLegacyBlockNoteDocument(JSON.parse(revision.content_json)),
          revision.section_id
        )
        ...
        if (result.changes !== 1) {
          throw new Error('Native block Math and Diagram migration lost its revision CAS')
        }
      ...
      if (foreignKeyViolations.length > 0) {
        throw new Error(
          `Native block Math and Diagram migration foreign key check failed: ${JSON.stringify(foreignKeyViolations)}`
        )
      }
      const integrity = database.pragma('integrity_check', { simple: true })
      if (integrity !== 'ok') {
        throw new Error(
          `Native block Math and Diagram migration integrity check failed: ${String(integrity)}`
        )
      }
    } finally {
