...
export const migration0017: DatabaseMigration = {
  version: 17,
  name: '0017-mutation-proposal-results',
  checksum: 'sha256:83ef82c84eafc344165a1c83845ee842d121d2cfe3342351622779c7bc27f728',
  up(database) {
    const incompatible = database
      .prepare(
        `SELECT COUNT(*)
           FROM mutation_proposals
          WHERE (kind <> 'section_patch' AND status IN ('applied', 'undone'))
             OR (kind = 'section_patch' AND base_revision_id IS NULL)
             OR (kind = 'brief_update' AND base_brief_version IS NULL)
             OR (kind = 'outline_patch' AND base_outline_version IS NULL)`
      )
      .pluck()
      .get() as number
    if (incompatible !== 0) {
      throw new Error('Cannot refine mutation proposal results with incompatible persisted rows')
    }

    database.exec(`
      DROP INDEX mutation_proposals_run_idx;
      DROP INDEX mutation_proposals_status_idx;
      ALTER TABLE mutation_proposals RENAME TO mutation_proposals_v16;
      ...
      SELECT
        ...
        agent_tool_call_id, kind, payload_json, base_revision_id, base_brief_version,
        base_outline_version, status, decision_at, applied_revision_id,
        NULL, NULL, NULL, rejected_reason, created_at, updated_at
      FROM mutation_proposals_v16;

      DROP TABLE mutation_proposals_v16;
