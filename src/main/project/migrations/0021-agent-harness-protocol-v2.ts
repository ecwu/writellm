...
export const migration0021: DatabaseMigration = {
  version: 21,
  name: '0021-agent-harness-protocol-v2',
  checksum: 'sha256:3cb24cc56c09c55c476d9b4ffdf276050c561975129dcc537e8c0d8a066209a8',
  up(database) {
    database.exec(`
      ...
      ALTER TABLE mutation_proposals RENAME TO mutation_proposals_v20;
      ALTER TABLE agent_events RENAME TO agent_events_v20;
      ...
      ORDER BY agent_session_id, sequence;
      ...
      CREATE TABLE mutation_proposals (
        ...
        CHECK (
          ...
          OR (kind = 'section_patch' AND base_revision_id IS NOT NULL
        ...
        CHECK (
          (status IN ('pending', 'approved', 'rejected', 'failed', 'superseded', 'conflicted', 'satisfied')
          ...
          OR (status = 'applied' AND undo_revision_id IS NULL AND (
            ...
            OR (kind = 'section_patch' AND applied_revision_id IS NOT NULL
              AND applied_brief_version IS NULL AND applied_outline_version IS NULL)
          ))
          OR (status = 'undone' AND kind = 'section_patch'
      ...
      SELECT
        ...
        applied_brief_version, applied_outline_version, undo_revision_id,
        replaces_proposal_id, rejected_reason, created_at, updated_at
      FROM mutation_proposals_v20;

      DROP TABLE mutation_proposals_v20;
      ...
      UPDATE agent_sessions SET event_schema_version = 2 WHERE event_schema_version < 2;
    `)
  }
}
