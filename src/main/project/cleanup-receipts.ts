...
export class CleanupReceipts {
  ...
  async load(): Promise<void> {
    try {
      ...
      if (
        !isRecord(parsed) ||
        parsed.kind !== CLEANUP_INDEX_KIND ||
      ...
      const receipts = parsed.receipts as CleanupReceipt[];
      const identities = receipts.map((receipt) => `${receipt.finalRoot}\0${receipt.token}`);
  ...
  async add(finalRoot: string): Promise<CleanupReceipt> {
    ...
    await writeAtomicJson(this.indexPath, {
      kind: CLEANUP_INDEX_KIND,
  ...
  async remove(receipt: CleanupReceipt): Promise<void> {
    const next = this.receipts.filter(
      (candidate) => candidate.token !== receipt.token || candidate.finalRoot !== receipt.finalRoot,
    );
    await writeAtomicJson(this.indexPath, {
      kind: CLEANUP_INDEX_KIND,
      schemaVersion: PROJECT_SCHEMA_VERSION,
...
function isReceipt(value: unknown): value is CleanupReceipt {
  return (
    isRecord(value) &&
    typeof value.finalRoot === 'string' &&
    path.isAbsolute(value.finalRoot) &&
    isUuid(value.token) &&
