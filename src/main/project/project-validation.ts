...
export type ValidationFailure = {
  code: Extract<
    ProjectErrorCode,
    'PROJECT_INVALID' | 'PROJECT_UNSUPPORTED_VERSION' | 'PROJECT_NOT_FOUND' | 'PROJECT_INACCESSIBLE'
  >;
  message: string;
};
...
export function validateProjectName(
  displayName: unknown,
): { ok: true } | { ok: false; code: 'INVALID_PROJECT_NAME'; message: string } {
  ...
  if (
    ...
    return {
      ...
      message: 'Project name must be a single filesystem name.',
    };
  }
...
export function validateManifest(
  ...
  | { ok: false; code: 'PROJECT_INVALID' | 'PROJECT_UNSUPPORTED_VERSION'; message: string } {
  if (!isRecord(value))
    return { ok: false, code: 'PROJECT_INVALID', message: 'Project manifest is not an object.' };
  if (value.schemaVersion !== PROJECT_SCHEMA_VERSION) {
    return {
      ok: false,
      code: 'PROJECT_UNSUPPORTED_VERSION',
      message: 'Project format version is not supported.',
    };
  }
  if (
    value.kind !== PROJECT_KIND ||
    !isUuid(value.projectId) ||
    typeof value.displayName !== 'string' ||
    !validateProjectName(value.displayName).ok
  ) {
    return { ok: false, code: 'PROJECT_INVALID', message: 'Project manifest identity is invalid.' };
  }
  if (
    !Array.isArray(value.requiredDirectories) ||
    value.requiredDirectories.length !== 1 ||
    value.requiredDirectories[0] !== REQUIRED_PROJECT_DIRECTORIES[0]
  ) {
    return {
      ok: false,
      code: 'PROJECT_INVALID',
      message: 'Project required directories are invalid.',
    };
  }
  if (
...
export async function validateProjectDirectory(projectRoot: string): Promise<ProjectValidation> {
  try {
    ...
    if (!root.isDirectory())
      return invalid('PROJECT_INVALID', 'Selected project is not a directory.');
  } catch (error) {
  ...
  try {
    ...
    if (isNodeError(error) && error.code === 'ENOENT')
      return invalid('PROJECT_INVALID', 'Project manifest is missing.');
  ...
  try {
    ...
    if (!workspace.isDirectory())
      return invalid('PROJECT_INVALID', 'Project workspace directory is missing.');
  } catch (error) {
    if (isNodeError(error) && error.code === 'ENOENT')
      return invalid('PROJECT_INVALID', 'Project workspace directory is missing.');
  ...
  return {
    ...
    project: {
      projectId: manifestResult.manifest.projectId,
      displayName: manifestResult.manifest.displayName,
    },
  };
}

function invalid(
  code: 'PROJECT_INVALID' | 'PROJECT_UNSUPPORTED_VERSION',
  message: string,
): ProjectValidation {
...
function filesystemFailure(
  error: unknown,
  fallback = 'Project could not be verified.',
): ProjectValidation {
  const code =
    isNodeError(error) && error.code === 'ENOENT' ? 'PROJECT_NOT_FOUND' : 'PROJECT_INACCESSIBLE';
  return {
    ok: false,
    error: { code, message: code === 'PROJECT_NOT_FOUND' ? 'Project was not found.' : fallback },
  };
}
