...
export class ProjectGitRepository {
  async commitContent(projectRoot: string, relativePath: string, revision: number): Promise<void> {
    ...
    if (!fs.existsSync(gitDir)) {
      await writeFile(path.join(projectRoot, '.gitignore'), ignore, { encoding: 'utf8', flag: 'wx' }).catch(error => { if ((error as NodeJS.ErrnoException).code !== 'EEXIST') throw error; });
      await writeFile(path.join(projectRoot, '.gitattributes'), '*.json text eol=lf\n*.md text eol=lf\n*.pdf binary\n', { encoding: 'utf8', flag: 'wx' }).catch(error => { if ((error as NodeJS.ErrnoException).code !== 'EEXIST') throw error; });
