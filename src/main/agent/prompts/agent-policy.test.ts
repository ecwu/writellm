...
describe('Agent writing policy', () => {
  it('keeps collaboration, academic-writing, and citation requirements explicit and bounded', () => {
    ...
    expect(policy).toContain('Writing Rules are trusted project conventions')
    expect(policy).toContain('report the conflict instead of silently choosing one')
    ...
    expect(policy).not.toContain('REVIEW_POLICY')
    expect(policy).not.toContain('check_draft')
    expect(policy).not.toContain('review issues')
  ...
  it('places the immutable mode ceiling after application safety and before collaboration', () => {
    ...
    expect(plan).not.toContain('review issues')
