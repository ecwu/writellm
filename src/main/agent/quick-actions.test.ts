...
describe('Agent quick action selection validation', () => {
  it('accepts an exact partial selection in ordered canonical blocks', () => {
    expect(validateQuickActionSelection(manuscript(), context('opening sentence'), log)).toEqual({
  ...
  it('fails before a run when revision, text, block identity, or order is stale', () => {
    expect(() =>
      validateQuickActionSelection(manuscript(nextRevisionId), context('opening sentence'), log)
    ).toThrow('changed after capture')
    expect(() => validateQuickActionSelection(manuscript(), context('missing words'), log)).toThrow(
      'text changed'
    )
    expect(() =>
      validateQuickActionSelection(manuscript(), context('opening', ['missing-block']), log)
