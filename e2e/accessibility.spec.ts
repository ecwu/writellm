...
test(
  ...
  async ({ testRoot }, testInfo) => {
    ...
    try {
      ...
      await expect(dialog.getByRole('row')).toHaveCount(18)
      await expect(dialog.getByText('Toggle project sidebar', { exact: true })).toHaveCount(0)
      await expect(dialog.getByText('Toggle writing Agent', { exact: true })).toHaveCount(0)
