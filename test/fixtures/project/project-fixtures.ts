...
export async function createValidProject(parent: string, displayName = 'Example'): Promise<{ root: string; manifest: ProjectManifest }> {
  const root = path.join(parent, `${displayName}.writellm`);
  const createdAt = new Date('2026-07-12T00:00:00.000Z').toISOString();
  const manifest: ProjectManifest = { kind: 'writellm.project', schemaVersion: 1, projectId: randomUUID(), displayName, requiredDirectories: ['workspace'], createdAt, updatedAt: createdAt };
