...
function dialogFor(pathValue: string): DirectoryDialog { return { showOpenDialog: async () => ({ canceled: false, filePaths: [pathValue] }) }; }
...
test('direct open after a move preserves identity, recent ID, bytes, and unknown files', async () => {
  ...
  const first = new ProjectRepository({ userDataPath: data, dialog: dialogFor(fixture.root), now: () => '2026-07-12T00:00:00.000Z' });
  ...
  const reopened = new ProjectRepository({ userDataPath: data, dialog: dialogFor(movedRoot), now: () => '2026-07-12T00:01:00.000Z' });
  await reopened.initialize();
  expect(await reopened.openProjectFromDialog()).toMatchObject({ status: 'opened', project: { projectId: fixture.manifest.projectId } });
  const listed = await reopened.listRecentProjects();
  expect(listed.recentProjects[0]).toMatchObject({ recentId, projectId: fixture.manifest.projectId, displayName: 'Portable' });
...
test('twenty move and reopen iterations preserve the stable project identity', async () => {
  ...
  for (let iteration = 0; iteration < 20; iteration += 1) {
    ...
    const repository = new ProjectRepository({ userDataPath: data, dialog: dialogFor(target), now: () => new Date(2026, 6, 12, 0, 0, iteration).toISOString() });
  ...
  expect((await import('node:fs/promises').then(({ stat }) => stat(current))).isDirectory()).toBe(true);
});
