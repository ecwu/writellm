...
import { validateManifest, validateProjectDirectory, validateProjectName } from '../../../src/main/project/project-validation';
...
test('validates a v1 manifest and required workspace directory', async () => {
  ...
  expect(validateManifest(JSON.parse(await readFile(path.join(fixture.root, 'project.json'), 'utf8'))).ok).toBe(true);
});
...
test('rejects unknown schema and missing required directory without repair', async () => {
  ...
  expect(await validateProjectDirectory(fixture.root)).toMatchObject({ ok: false, error: { code: 'PROJECT_UNSUPPORTED_VERSION' } });
  await writeFile(manifestPath, original);
  await import('node:fs/promises').then(({ rm }) => rm(path.join(fixture.root, 'workspace'), { recursive: true }));
  expect(await validateProjectDirectory(fixture.root)).toMatchObject({ ok: false, error: { code: 'PROJECT_INVALID' } });
