...
test('relink mismatch preserves the original recent record', async () => {
  ...
  const openRepository = new ProjectRepository({ userDataPath: data, dialog: dialogFor(original.root), now: () => '2026-07-12T00:00:00.000Z' });
  ...
  const relinkRepository = new ProjectRepository({ userDataPath: data, dialog: dialogFor(other.root), now: () => '2026-07-12T00:01:00.000Z' });
  await relinkRepository.initialize();
  const recentId = before.recentProjects[0].recentId;
  expect(await relinkRepository.relinkRecentProject(recentId)).toMatchObject({ status: 'error', error: { code: 'PROJECT_ID_MISMATCH' } });
  expect(await relinkRepository.listRecentProjects()).toMatchObject({ recentProjects: [{ recentId, projectId: original.manifest.projectId, availability: 'available' }] });
});
