...
test('save, edit and restart remain application-global and leave project bytes unchanged', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'provider-flow-'));
  const app = path.join(root, 'userData');
  const project = path.join(root, 'project.txt');
  await writeFile(project, 'project-sentinel');
  let n = 0;
  ...
  const first = await repo.save(null, config, 'secret');
  expect(first.ok).toBe(true);
  const second = await repo.save('r1', { ...config, modelId: 'writer-2' });
  expect(second.ok).toBe(true);
  const reopened = new ProviderSettingsRepository(app, protector);
  await reopened.initialize();
  expect(reopened.summary()).toMatchObject({
    revision: 'r2',
    config: { modelId: 'writer-2' },
